...
| M15 | **Genom QC (§0.2)** — BV-BRC metadata ön-filtre + yerel CheckM2 + QUAST | 🟡 **SCRIPT WRITTEN, NOT RUN** — `02d_genome_qc.py` (prep/post: CheckM2 amr-checkm2.sif + QUAST amr-tools.sif; completeness/contamination/N50/contig thresholds → per-genome table + pass-rate JSON + advisory exclusion list) + `test_02d_genome_qc.py`. Needs CheckM2 DB download + one SLURM run on the 5468 assemblies. (02b IQR advisory still in place too.) | 3 gün |
