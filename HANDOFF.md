...
**STRATEGIC PIVOT (agreed): stop optimising the prediction engine; move to the thesis contribution.** Per `docs/ROADMAP.md` the research question is the **queryable, biologically-validated AMR k-mer Knowledge Base**, NOT prediction accuracy — AUC 0.93 is already more than enough to rank k-mers for the KB. Engine is now consolidated/audited (see §9 latest entry); next is the biology/KB chain `07b → 07 → 08 → 09 → 10 → 11` + the KB persistence layer (M8) + cross-antibiotic (S1/H3) + permutation test (M9). Final full-data model (all train chunks, `external_memory: true`, `max_train_chunks: null`) to be trained, then proceed to biology.
