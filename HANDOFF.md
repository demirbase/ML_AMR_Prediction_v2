...
**IMMEDIATE NEXT (resume here):** Implement ROADMAP §0, in order: (1) ✅ **unitig pipeline** (DONE — §0.1); (2) ✅ **PopPUNK lineage + GroupKFold** (DONE — §0.1); (3) **CheckM2/QUAST genome QC** + BV-BRC metadata filter (00a); (4) **CPSS+SHAP** rewrite of 07b; (5) **external validation** (temporal/geo hold-out + AMRFinderPlus); (6) **pyseer LMM**, MDA permutation, BH-FDR (10), ARO mapping in KB; (7) run `07→08→09→10→11` biology on unitigs + populate the KB; (8) FastAPI (S8). All on the CPU `$SIF` (`amr.sif`/`amr-pp.sif`); engine code consolidated/audited on `main`. **CURRENTLY: unitig ML run (04→05→06→07b) in progress; next is Block 1 (07→11 biology on unitigs).**

## §0.1 Unitig + lineage-CV — DONE (2026-06-23, IMMEDIATE NEXT steps 1 & 2)

**Containers (TRUBA, build on a debug node: `unset APPTAINER_BINDPATH` — `/arf` can't bind inside the build sandbox; login node hits a CPU-time ulimit on `mksquashfs`):**
- `amr.sif` — CPU pipeline + `unitig-caller 1.3.2 (Bifrost)` + `bcalm`.
- `amr-pp.sif` — same + `poppunk 2.7.8` (env pins `setuptools<81`: PopPUNK imports legacy `pkg_resources`, removed in setuptools 81). Built to a new name, kept separate.
- **Container rebuild #2 (pending, bundle in ONE build):** `pyseer + checkm2 + quast + amrfinderplus + resfinder` for the remaining MUST-haves (M13/M14/M15). Some download DBs (CheckM2 ~1.4 GB, AMRFinderPlus) → longer build.

**STEP 1 — Unitig matrix (`scripts/03u_unitig_matrix.py`):** `unitig-caller --call --rtab` → the EXACT chunked contract of 03 (`features.txt` [seq+`\t1`], `y_{ab}.csv`, `genomes_{ab}.csv`, `X_{ab}_part_*.npz`) so 03b/04/05/06/07/07b read it unchanged. rtab sample cols→rows mapped by HEADER name; absolute `min_support` + zero-variance-core drop. Writes to `matrix_unitig/` (config `unitig.out_subdir`), raw-k-mer baseline preserved. **Real ampicillin run done: 4,938,938 unitigs × 4373 genomes, 22 chunks, ~8 GB** (vs 21.4M k-mers / 49 GB — ~4.3× smaller). NOTE: the unitig-caller `--rtab` was 70 GB (dense; pyseer sparse format is LARGER, not the answer); the run took ~3 h (graph build + 70 GB write + parse).
- **ORGANISM-LEVEL store (`--build-db`):** unitigs are antibiotic-independent → run unitig-caller ONCE over all the organism's `.fna` → `processed/{organism}/unitig_all/`; each antibiotic then `03u --antibiotic X` SUBSETS the store (select rows + re-filter `min_support`) with NO unitig-caller re-run (`subset_store_to_antibiotic`). Config `unitig.db_min_support` (store floor, default 2, safe ≤ every antibiotic's min_support). For E. coli use `--build-db --db-min-support 10`. The ampicillin matrix above was the per-antibiotic fallback path (no store yet); build the store before the 2nd antibiotic.

**STEP 2 — Lineage-aware CV (ROADMAP §0.1 M2, DONE):**
- `scripts/02c_lineage_poppunk.py` — PopPUNK organism-level (`--create-db` → `--fit-model dbscan`) → canonical `processed/{organism}/lineage/poppunk_clusters.csv` (`Genome ID,Cluster`). **bgmm was degenerate** (one ~94% mega-cluster + refine crashed on a NaN boundary); **dbscan gives 324 well-distributed lineages** (largest ~15%). Reverses PopPUNK's `.`→`_` Taxon mangling (`normalize_clusters`). Config `lineage:` (`model: dbscan`, `refine: false`, `n_splits: 5`, `min_cluster_size`). `--reuse-db` skips re-sketching; `--clusters-csv` normalises an existing fit.
- `scripts/lib/lineage.py` — `load_lineage`, `group_kfold_masks` (StratifiedGroupKFold → sample-level masks for `lib.xgb_data` row_mask), `collapse_rare_clusters`, `lineage_summary`. **Validated on real ampicillin data: 5 folds, all leakage-free, balanced R%.**
- `07b_feature_stability.py` — `build_cv_splits()`: lineage-aware StratifiedGroupKFold when `poppunk_clusters.csv` exists (honest AUC mean±std + lineage-disjoint selection frequency), else falls back to 5-seed holdout. Output files/columns unchanged (summary `seed` column now holds fold ids).

**WIRING — `feature_repr` switch:** config `preprocessing.feature_repr: kmer|unitig` OR env `AMR_FEATURE_REPR=unitig` makes `resolve_path('matrix_dir')` redirect to `matrix_unitig/` — 04/05/06/07/07b/03b read unitigs with NO per-script change. Default `kmer` (baseline untouched). `chunk_size` for 03u MUST equal `preprocessing.chunk_size`.

**CURRENT JOB:** unitig ML run (`run_ml_unitig.slurm`, `export AMR_FEATURE_REPR=unitig`): `04 → 05 → 06 → 07b` on the unitig matrix. 06 = chunk-split AUC (optimistic, compare to k-mer 0.930); **07b = lineage-aware GroupKFold mean±std = the honest thesis headline.** NOTE: outputs (models/results/experiment_config) OVERWRITE the k-mer ones (acceptable; k-mer headline documented). external_memory left `true` (ExtMem); unitig matrix is small enough for in-core (`false`, faster) — optional optimisation.

**NEXT after this job:** Block 1 — run `07→08→09→10→11` biology on unitigs (07 Gain→**SHAP** M4; 08 normal blastn not blastn-short since unitigs are long; 09 **ARO mapping** M16 + reification wording S10; 10 **BH-FDR** §0.2; 11 ciprofloxacin gyrA). Block 2 — MUST-haves needing container #2: M4 CPSS, M14 pyseer LMM, M9 permutation, M13 external val (AMRFinderPlus/ResFinder + temporal/geo), M15 CheckM2/QUAST QC; then M6/M10 CARD version + Zenodo, M8 KB populate on real unitig outputs, S8 FastAPI.

**TRUBA config note:** the TRUBA `config.yaml` is manual (not committed); the new `unitig:`/`lineage:` sections were added by hand. New code that needs a config key falls back gracefully (02c/03u derive `lineage_dir`/store from `data_dir`; feature_repr via env). Pull code with targeted `git checkout origin/main -- <file>`, NEVER `config.yaml` (would clobber HPC tuning) and NEVER the manually-patched `02p`/parallel `02b`/`03`.
...
- **2026-06-23 unitig + lineage-CV DONE (ROADMAP §0 IMMEDIATE NEXT #1 & #2):** see **§0.1** for the full status. Highlights: `03u_unitig_matrix.py` (unitig-caller → 03-contract matrix; ampicillin = **4.94M unitigs**, ~8 GB vs 21.4M k-mers/49 GB; + organism-level `--build-db` store so the 2nd antibiotic just subsets); `02c_lineage_poppunk.py` + `lib/lineage.py` (PopPUNK **dbscan** → 324 lineages; bgmm was a 94% mega-cluster + refine crashed; StratifiedGroupKFold masks validated leakage-free on real data); `07b` rewired to lineage-aware CV (5-seed fallback kept); `feature_repr` switch (config or `AMR_FEATURE_REPR` env) redirects `matrix_dir`→`matrix_unitig` with zero per-script change. Two containers: `amr.sif` (+unitig-caller/bcalm), `amr-pp.sif` (+poppunk, `setuptools<81`). 77 non-integration + 1 integration pass. Commits `0909589`→`36be079` on `main`. Unitig ML run (04→05→06→07b) in progress on TRUBA.
