...
**IMMEDIATE NEXT (resume here):** ROADMAP §0 order: (1) ✅ **unitig pipeline**; (2) ✅ **PopPUNK lineage + GroupKFold**; (1.5) ✅ **unitig ML run 04→05→06→07b** (RESULTS in §0.1); (Block 1) ✅ **07→11 biology on unitigs DONE & reproducible** (NCBI remote BLAST fixed — see §0.1 "Block 1 + KB DONE"); (M8) ✅ **KB populated** (`results/ecoli/kb/amrk.db`, schema 0.2.0); (M16) ✅ **ARO ontology in KB**; (M6) ✅ **CARD 4.0.1 recorded**; (M9) ✅ **DONE & saved** (MDA `12` + label-perm `12b`: REAL 0.9534 ≫ null ~0.50, **p=0.0099 significant** — see §0.1 "M9"). (M11) ✅ permutation evidence in KB; (M4) ✅ **CPSS + SHAP DONE** (`13`/`13b`: 39 stable, PFER≤5.4, 5 confirmed TEM/aminoglycoside full-coverage, in KB schema 0.3.0 as method='cpss'); (M14) ✅ **pyseer LMM DONE** (`14`: subsampled kinship + candidate LMM; 26/39 CPSS-stable lineage-corrected significant; **3/3 TEM β-lactamase pass, aminoglycoside co-resistance does NOT** — LMM separates mechanism from co-resistance; in KB as `pyseer_lmm` evidence, TEM p≈1e-109). **KB now carries 275 validation_evidence rows across 7 evidence types.** **→ CONTINUE Block 2:** (a) M13 external validation (temporal/geo hold-out + AMRFinderPlus/ResFinder concordance, `amr-tools.sif`); (b) M15 CheckM2/QUAST QC (`amr-checkm2.sif`); then M10 provenance (run_metadata/git hash — KB run_id still `…__unknown`), S8 FastAPI, S1 cross-antibiotic. S10 reification + BH-FDR(10) already coded.
