...
**IMMEDIATE NEXT (resume here): rebuild 03 with adaptive min_support, then the full pipeline on the smaller feature set.** Config to use: `preprocessing.min_support: null` (→44 for ampicillin), `xgboost_params.device: cpu`, `training.external_memory: true`, `training.max_train_chunks: null`. Steps: re-run **03** (matrix, ~17–24M features, faster — smaller vocab dict) → 03b → **04** (HPO) → **05** (full-data boosting + early stopping) → **06** → **07b** (5-seed, now feasible ~12–17 h) → **biology/KB chain 07→08→09→10→11** → KB layer (M8). Use the CPU `$SIF` (`amr.sif`); the engine code is consolidated/audited (HEAD on `main`).
