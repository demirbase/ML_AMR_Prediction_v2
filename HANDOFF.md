...
**H) Block-2 tool runs** (`SIF=amr-tools.sif`/`amr-checkm2.sif`): pyseer LMM (M14) ✓, **AMRFinderPlus/ResFinder concordance (M13) ✓** (`16_external_concordance.py`; amrfinder DB via `amrfinder_update -d $AMR_WORK/data/external/amrfinder_db`, ResFinder DBs cloned to `$AMR_WORK/data/external/{resfinder,pointfinder}_db`; per-genome loop `xargs -P20` inside one `amr-tools.sif` exec over `16_genomes.txt`, then `--mode post` in `amr.sif`). CheckM2+QUAST QC (M15) — **script IS written** (`02d_genome_qc.py`, prep/post + `test_02d_genome_qc.py`), **not yet run**: needs CheckM2 DB (`checkm2 database --download --path $AMR_WORK/data/external/checkm2_db`), then `02d --mode prep` → `checkm2 predict` (amr-checkm2.sif) + `quast.py` (amr-tools.sif) → `02d --mode post`.
