...
**IMMEDIATE NEXT (resume here):** Implement ROADMAP §0, in order: (1) **unitig pipeline** (bcalm2 + unitig-caller → unitig matrix; KMC stays for QC only) — **IN PROGRESS, see §0.1 below**; (2) **PopPUNK lineage labels + GroupKFold** integration into 04/05; (3) **CheckM2/QUAST genome QC** + BV-BRC metadata filter (00a); (4) **CPSS+SHAP** rewrite of 07b; (5) **external validation** (temporal/geo hold-out + AMRFinderPlus); (6) **pyseer LMM**, MDA permutation, BH-FDR (10), ARO mapping in KB; (7) re-run end-to-end on unitigs and populate the KB; (8) FastAPI (S8). All on the CPU `$SIF` (`amr.sif`); engine code consolidated/audited on `main`.

## §0.1 Unitig pipeline — progress (2026-06-22, step 1 of IMMEDIATE NEXT)

**Done & committed/pushed (`main`):**
- `environment.yml` adds `bcalm` + `unitig-caller` (commit `0909589`). **TRUBA container `amr.sif` REBUILT** from `amr.def` on a debug node (login node hits a CPU-time ulimit on the final `mksquashfs`; build with `unset APPTAINER_BINDPATH` — `/arf` can't be bind-mounted inside the build sandbox). Now ships **unitig-caller 1.3.2 (Bifrost backend) + bcalm**.
- Smoke (20 genomes) confirmed the rtab format: line 1 header `Unitig_sequence<TAB>{genome_id}…`; data lines `{unitig_seq}<TAB>0/1…` in header column order; sample names == genome_id (basename, no `.fna`).

**Built & tested, NOT yet committed (pending push approval):**
- `scripts/03u_unitig_matrix.py` — converts `unitig-caller --call --refs <one-abs-path-per-line> --rtab` output into the **exact same chunked matrix contract as 03** (`features.txt` [unitig seq + `\t1`], `y_{ab}.csv`, `genomes_{ab}.csv`, `X_{ab}_part_*.npz`), so 03b/04/05/06/07/07b read it unchanged. Maps rtab sample columns→rows by HEADER name (no order assumption); absolute `min_support` + zero-variance-core drop (ROADMAP §0.7, NOT proportional). Writes to a SEPARATE `unitig.out_subdir` (default `matrix_unitig/`) so the raw-k-mer baseline is preserved. Config: new `unitig:` section (`out_subdir`, `min_support: 10`, `threads`). `tests/test_03u_unitig_matrix.py` (3 tests); 60 non-integration tests pass; py_compile clean (3.10).

**DELIBERATE TODO (do NOT forget):** downstream 04/05/03b still `resolve_path('matrix_dir')` → `…/matrix` (raw k-mers). To train on unitigs, add a `feature_repr: kmer|unitig` switch (or `matrix_subdir`) so `matrix_dir` resolves to `matrix_unitig/` — deferred to IMMEDIATE NEXT step 7 (re-run end-to-end on unitigs), to avoid touching shared `resolve_path` prematurely. `chunk_size` for 03u MUST equal `preprocessing.chunk_size` (04/05/06 slice `y_{ab}.csv` by it).

**Run 03u on TRUBA (after pull):** `git checkout origin/main -- scripts/03u_unitig_matrix.py config/config.yaml environment.yml`; barbun job (`-c 40`, big `--mem`) `apptainer exec $SIF python scripts/03u_unitig_matrix.py` (defaults: ampicillin, `min_support 10`). Validate parser on real output first (cheap): `--rtab smoke_unitigs2.rtab` against a matching genome subset.
...
- **2026-06-22 unitig pipeline — step 1 started (ROADMAP §0 / IMMEDIATE NEXT #1):** added `bcalm`+`unitig-caller` to `environment.yml` (pushed `0909589`); rebuilt the TRUBA Apptainer container (now unitig-caller 1.3.2 + bcalm); confirmed the unitig-caller `--rtab` output format on a 20-genome smoke; wrote `scripts/03u_unitig_matrix.py` (rtab→03-contract chunked matrix into a separate `matrix_unitig/`, header-based sample→row mapping, absolute min_support) + `tests/test_03u_unitig_matrix.py`; added the config `unitig:` section. 60 non-integration tests pass. **Full details + the deliberate downstream-wiring TODO in §0.1.** NOT yet committed (03u/test/config/docs pending push).
