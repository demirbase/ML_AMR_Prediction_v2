...
**IMMEDIATE NEXT (resume here):** Implement ROADMAP §0, in order: (1) **unitig pipeline** (bcalm2 + unitig-caller → unitig matrix; KMC stays for QC only); (2) **PopPUNK lineage labels + GroupKFold** integration into 04/05; (3) **CheckM2/QUAST genome QC** + BV-BRC metadata filter (00a); (4) **CPSS+SHAP** rewrite of 07b; (5) **external validation** (temporal/geo hold-out + AMRFinderPlus); (6) **pyseer LMM**, MDA permutation, BH-FDR (10), ARO mapping in KB; (7) re-run end-to-end on unitigs and populate the KB; (8) FastAPI (S8). All on the CPU `$SIF` (`amr.sif`); engine code consolidated/audited on `main`.
