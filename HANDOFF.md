...
**OPEN PROBLEM (next priority): metrics likely capped by UNDERFITTING, not features.** Evidence: test probability range is compressed to **[0.29, 0.72]** with only **n_estimators=30 trees** (lr≈0.02). AUC (rank-based) is fine (0.93) but threshold metrics (MCC/acc at 0.5) suffer because probabilities cluster near 0.5. Root cause = **n_estimators is chosen by early-stopping on the tiny 480-sample HPO subset, which is far too few trees for the 3573-sample full-data training**. This is **systemic — it will hurt every antibiotic/organism**. Proposed generic fix: let **05 run its own early stopping on a validation split of the FULL train set** (high `num_boost_round` + `early_stopping_rounds`) so the tree count adapts to the real training size; keep HPO for the other HPs (lr/depth/etc.). Secondary levers: bigger HPO subset (`training.optuna_fraction`), then `min_support` feature pruning (needs 03 re-run). **Biology chain `07b → 07 → 08 → 09 → 10 → 11` NOT yet run** for this dataset — do after the underfit fix so stability/biology reflect the better model.
