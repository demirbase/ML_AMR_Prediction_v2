...
**H) Block-2 tool runs** (`SIF=amr-tools.sif`/`amr-checkm2.sif`): pyseer LMM (M14) ✓, **AMRFinderPlus/ResFinder concordance (M13) ✓** (`16_external_concordance.py`; amrfinder DB via `amrfinder_update -d $AMR_WORK/data/external/amrfinder_db`, ResFinder DBs cloned to `$AMR_WORK/data/external/{resfinder,pointfinder}_db`; per-genome loop `xargs -P20` inside one `amr-tools.sif` exec over `16_genomes.txt`, then `--mode post` in `amr.sif`). CheckM2+QUAST QC (M15) — script not written yet.
