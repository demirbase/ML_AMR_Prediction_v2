...
**IMMEDIATE NEXT (resume here):** ROADMAP §0 order: (1) ✅ **unitig pipeline**; (2) ✅ **PopPUNK lineage + GroupKFold**; (1.5) ✅ **unitig ML run 04→05→06→07b** (RESULTS in §0.1); (Block 1) 🔄 **07→11 biology on unitigs** (08/09 on UI for NCBI; 10/11 on compute) — IN PROGRESS; (3) CheckM2/QUAST QC (00a); (4) CPSS+SHAP rewrite of 07b; (5) external validation (temporal/geo + AMRFinderPlus/ResFinder); (6) pyseer LMM, MDA permutation; (7) **M8 KB populate** on real unitig outputs; (8) FastAPI (S8). M16 ARO + S10 reification + BH-FDR(10) already coded.

## §0.1 Unitig + lineage-CV + biology — STATUS (2026-06-23)
